...
a) Extract all the elements of the first 10 rows and the first 4 columns of the `cardiac` dataframe and assign to a new variable called `cardiac.sub`. 

b) Next, extract all observations (remember - rows) from the `cardiac` dataframe and the columns `patno`, `sex`, `smoking` and `tchol` and assign to a variable called `cardiac.risk`. 

c) Now, extract the first 50 rows and all columns from the original dataframe and assign to a variable `cardiac.50` (there's a better way to select rows with conditional statements - you'll meet it in Exercise 4). 
