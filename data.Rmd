...
list.files(dir) %>%
  ...
  grep(paste("cardiacdata\\.xlsx", "cardiac_followup\\.txt", "cardiac_admissions\\.txt",
             "scotpho_alcohol_admissions\\.txt",
             "flower\\.xlsx", "problem_data\\.xlsx", sep = "|"), ., value = TRUE, invert = FALSE) %>% 
  paste0("    + [", ., "](", paste0(dir, .), ")") %>% 
  cat(sep = "\n")
```
