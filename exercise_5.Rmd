...
```{r Q8, echo=SOLUTIONS, eval=SOLUTIONS, fig.width=8, fig.height=8}
par(mfrow = c(2, 2))
...
```{r Q8b, echo=SOLUTIONS, eval=SOLUTIONS, fig.width=8, fig.height=4}
...
```{r Q9, echo=SOLUTIONS, eval=SOLUTIONS, fig.width=8, fig.height=4}
par(mfrow = c(1, 2))
...
```{r Q9b, echo=SOLUTIONS, eval=SOLUTIONS, fig.width=9, fig.height=3.5}
...
One of the useful things about `pairs()` is that you can choose what goes into each panel. Modify your plot to add a smoother (a wiggly line) to the lower triangle, which makes the shape of each relationship much easier to see. The book and `?pairs` show you how. Then get the correlation coefficients themselves with the `cor()` function, which takes a whole dataframe and returns a matrix of them (you will need its `use` argument to cope with the missing values). Which pair of variables is most strongly related, and which variable is related to almost nothing?
...
```{r Q12, echo=SOLUTIONS, eval=SOLUTIONS}
