...
8\. So far the two files you have linked have both been one row per patient, which is the easy case. Real linkage is usually messier than that, because the thing you are linking to happens more than once.

Download *'cardiac_admissions.txt'* from the **[<i class="fa fa-download"></i> Data](data.html)** link, save it to your `data` directory and import it into a variable called `admissions`. This file holds one row per **hospital admission** during the ten year follow up, not one row per patient. Note that unlike the rest of the cardiac data, which is real, these admission records are simulated.

Use `str()` and `nrow()` to get your bearings. How many admissions are there, and how many different patients do they belong to? Then, before you join anything, check the key in both directions. Are there any patients in `cardiac` with no admissions at all? Are there any admissions belonging to a patient number that is not in `cardiac`, and if so, how would that happen?

Now join `cardiac` to `admissions` with `merge()` as you did in Q6, and look at the number of rows. Compare `mean(age)` before and after the join. Something has gone wrong. What?
...
9\. So the join in Q8 is not the one you wanted. What you actually want is to keep one row per patient and attach a *summary* of that patient's admissions, and to do that you have to summarise the admissions **before** you link them, not after.

a) Use `aggregate()`, as you did in Q3, to produce one row per patient from `admissions`, holding the number of admissions and the total number of days spent in hospital. Counting rows per group is what `FUN = length` is for, and total bed days is `FUN = sum` applied to `los`.

b) Now link that summary onto `cardiac`, keeping all 163 patients as you did in Q7. Check the row count.

c) Look at the patients who were never admitted. What has the join put in their count column, and is that the right value? This is the one place in this whole course where an `NA` does not mean 'we do not know'. Work out what it does mean here, decide what to do about it, and calculate the mean number of admissions per patient both ways to see how much the decision matters.

d) Finally, the question the linkage was for. Does the number of admissions differ by smoking status?
