...
8\. One last linkage, and this one has a sting in it. The patients were also followed up for hospital admissions over the same ten years. Download *'cardiac_admissions.txt'* from the **[<i class="fa fa-download"></i> Data](data.html)** link, save it to your `data` directory and import it into a variable called `admissions`. Note that unlike the rest of the cardiac data, which is real, these admission records are simulated.

Take a look at it with `str()`. It has already been summarised for you to one row per patient, holding the number of admissions and the total number of days spent in hospital. In real life you would often be handed one row per *admission* and have to summarise it yourself before you could link it, which is a job for `aggregate()`.

a) How many rows does `admissions` have, and how does that compare with the 163 patients in `cardiac`?

b) Link it onto `cardiac_all`, keeping all 163 patients exactly as you did in Q7, and check the row count. Then count how many patients came out with a missing `n_admissions`. Add that number to the number of patients who did get a count. Is the total 163, and if not, where has the difference gone?

c) Now the sting. Look at those patients with a missing `n_admissions`. Everywhere else in this course an `NA` has meant 'there should be a value here and we do not have it'. Does it mean that here? Work out what it does mean, calculate the mean number of admissions per patient both with and without doing something about it, and decide which answer you would report.
