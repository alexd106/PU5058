...
11\. Not every figure in a report will be one you constructed with R code. You might need a diagram, a photograph, a map, or a figure somebody else made (see [Section 8.5.4](https://intro2r.com/r-markdown-anatomy.html#adding-figures) of the book again). Add the plot you exported in **Exercise 6, Q9**, `ex6_admissions.png`, to your report. If you no longer have the file, [download it here](images/ex6_admissions.png) and save it into your `output` directory, the one you created in **Exercise 1, Q12**.

There are two ways of doing it and it's worth trying both.

a) The quick way needs no chunk at all, just a line of markdown in among your text. It's made of three pieces in a row with no spaces between them: an exclamation mark, then the caption you want in square brackets, then the path to the file in round brackets. Section 8.5.4 of the book has an example if you get stuck. Knit and check the figure appears.

b) The other way puts the file into a chunk instead, using the `include_graphics()` function from `knitr`, in the same way you would write `knitr::kable()`. Give it the path to the file in quotes. On its own this looks like more work for the same result, but it buys you something: because the figure is now coming out of a chunk, all the chunk options are available to it. You can caption it with `fig.cap` exactly as you did in Q7, and you can set how much of the page width it takes up with `out.width`, written as a percentage in quotes such as `"80%"`. Try it with a caption and a width, and knit.

Which you use is a judgement call. The markdown line is quicker to write, and the chunk gives you control over how the figure is presented, which is usually what you want in a report.
